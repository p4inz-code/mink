...
**Session 109 updates (in progress):** R01 and T04 (interactive REPL) **P1 blockers CLOSED** — `mink repl` delivers a compile-eval session on the native backend, with an initial-file preload and `:help`/`:show`/`:clear`/`:quit` commands. Coverage: `tests/repl.rs` r01-r14 (native PE runs through the real CLI). Parity-blocking set: 18 → 16. Aggregate tables in §10.1/§10.2/§10.4/§10.6 are recomputed in the Session 109 completion pass (row flags above are authoritative meanwhile).
